 ...
 * @LastEditTime: 2025-12-01 18:48:53
 * @LastEditors: lkyezi
 * @Description: 
 * @
 */
...
typedef struct
{
    ...
    /* IPC的画面方向信息 */
    bool hanging_upright; // 默认镜像翻转时是否为 1:正装 0:吊装 需要根据机型赋值
...
typedef enum
{
    TRACKER_TIMER_AI_PTS = 0,
    TRACKER_TIMER_TARGET_LOST = 1,
...
public:
    ...
    // =============== 初始化与配置 (snake_case) ===============
    bool init(const char* config_file, c_media* media, const HardwareInterface* hw_override = nullptr);
    void shutdown();
    bool reload_config(const char* config_file);
    const PIDControlConfig* get_config() const;
    ...
    // =============== 目标更新（可直接外部喂入） ===============
    void update_target(float x_pixel, float y_pixel, bool visible, uint64_t pts, int32_t rid);
...
private:
    ...
    void install_hardware_interface(const HardwareInterface* hw_override);
    static int  motor_index(uint8_t axis);

    // ---- 回调静态封装 ----
    static bool get_target_pixel_callback(void* ctx, float* x_pixel, float* y_pixel, uint64_t* pts);
    static float read_angle_callback(void* ctx, uint8_t axis);
    static void motor_set_speed_callback(void* ctx, uint8_t axis, uint8_t level, uint8_t direction);
    static void motor_disable_callback(void* ctx, uint8_t axis);
    bool   get_target_pixel(float* x_pixel, float* y_pixel, uint64_t* pts) const;
    float  get_angle(uint8_t axis) const;
...
private:
    // ==== PID 底层 ====
    PIDControlLib* m_lib;
    PIDControlConfig* m_pid_config;
    HardwareInterface m_hw_interface;
    HardwareInterface* m_external_hw_interface;
    ...
    timer_ctrl* m_timers[TRACKER_TIMER_BUTT];
    ...
    // ==== 外部覆盖回调（可选）====
    GetTargetPixelPtsCallback m_external_target_cb;
    ReadAngleCallback m_external_read_angle_cb;
    MotorSetSpeedCallback m_external_motor_speed_cb;
    MotorDisableCallback m_external_motor_disable_cb;
